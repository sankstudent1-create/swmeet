...
export default function Signup() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (err) {
      ...
      setMsg("Success! Check your email to confirm your account, then log in.");
    }
  }
      ...
      <div className="w-full max-w-[420px] relative z-10 animate-fade-in stagger-2">
        <div className="glass-panel rounded-[2rem] p-8 sm:p-10 shadow-2xl border border-border/80">
          <div className="text-center mb-8">
            ...
            <p className="text-muted font-medium">Join SwMeet for free and start meeting.</p>
          </div>
          ...
          {msg && (
            <div className="mb-6 p-4 bg-success/10 border border-success/20 rounded-2xl flex items-start gap-3 animate-fade-in">
              <AlertCircle size={20} className="text-success shrink-0 mt-0.5" />
              <p className="text-sm font-semibold text-success">{msg}</p>
            </div>
          )}
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-bold uppercase tracking-widest text-muted pl-1">Password</label>
              <div className="relative">
                <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none text-muted">
                  <Lock size={18} />
                </div>
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full flex items-center justify-center gap-2 bg-brand text-white font-bold py-4 rounded-2xl shadow-md hover:bg-foreground hover:shadow-brand/30 transform hover:-translate-y-0.5 transition-all duration-300 mt-4 disabled:opacity-70 disabled:hover:transform-none disabled:hover:bg-brand"
            >
